...
async function handleSaveAdminPermissions() {
  try {
    ...
    if (!permissions || !Array.isArray(permissions)) {
      res.status(400).json({ error: '管理员权限数据格式错误' });
      return;
    }
    
    // 保存管理员权限数据 - 写操作是同步的，不需要await
    db.set('admin_permissions', JSON.stringify(permissions));
    
    res.status(200).json({ message: '管理员权限保存成功', permissions: permissions });
  } catch (error) {
    console.error('保存管理员权限失败:', error);
    res.status(500).json({ error: '保存管理员权限失败，请稍后重试' });
  }
}

// 执行保存管理员权限处理
if (req.method === 'POST') {
  handleSaveAdminPermissions().catch(error => {
    console.error('保存管理员权限处理失败:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: '保存管理员权限失败，请稍后重试' });
    }
  });
