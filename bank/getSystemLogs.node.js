...
async function handleGetSystemLogs() {
  try {
    ...
    // 获取系统日志数据 - 读操作是异步的，需要await
    const logsData = await db.get(`system_logs:${session.userId}`);
    ...
    if (logsData) {
      try {
        ...
        console.error('解析系统日志数据失败:', parseError);
        res.status(500).json({ error: '系统日志数据格式错误' });
      }
    } else {
      // 返回默认系统日志数组
    ...
    res.status(500).json({ error: '获取系统日志失败，请稍后重试' });
  }
}

// 执行获取系统日志处理
if (req.method === 'GET') {
  handleGetSystemLogs().catch(error => {
    console.error('获取系统日志处理失败:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: '获取系统日志失败，请稍后重试' });
    }
  });
