...
// 处理保存系统日志请求
async function handleSaveSystemLogs() {
  try {
    ...
    if (!logs || !Array.isArray(logs)) {
      res.status(400).json({ error: '系统日志数据格式错误' });
      return;
    }
    
    // 保存系统日志数据 - 写操作是同步的，不需要await
    db.set(`system_logs:${session.userId}`, JSON.stringify(logs));
    
    res.status(200).json({ message: '系统日志保存成功', logs: logs });
  } catch (error) {
    console.error('保存系统日志失败:', error);
    res.status(500).json({ error: '保存系统日志失败，请稍后重试' });
  }
}

// 执行保存系统日志处理
if (req.method === 'POST') {
  handleSaveSystemLogs().catch(error => {
    console.error('保存系统日志处理失败:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: '保存系统日志失败，请稍后重试' });
    }
  });
