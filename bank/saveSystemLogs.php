...
try {
    ...
    if ($method === 'POST') {
        ...
        if (!$logs || !is_array($logs)) {
            ...
            echo json_encode(['error' => '系统日志数据格式错误'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        $db->set("system_logs_{$session['userId']}", json_encode($logs));
        
        http_response_code(200);
        echo json_encode(['message' => '系统日志保存成功', 'logs' => $logs], JSON_UNESCAPED_UNICODE);
    ...
    error_log('保存系统日志失败: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => '保存系统日志失败，请稍后重试', 'details' => $e->getMessage()], JSON_UNESCAPED_UNICODE);
}
