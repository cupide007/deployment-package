...
async function handleGetCards() {
  try {
    ...
    // 获取银行卡片数据 - 读操作是异步的，需要await
    const cardsData = await db.get(`bank_cards:${session.userId}`);
    
    if (cardsData) {
      // 返回响应 - res是全局变量
      res.status(200).json({ cards: JSON.parse(cardsData) });
    } else {
      // 返回默认银行卡片数组
...
if (req.method === 'GET') {
  // 处理异步函数的错误
  handleGetCards().catch(error => {
    console.error('获取银行卡片处理失败:', error);
    // 如果响应尚未发送，则发送错误响应
    if (!res.headersSent) {
      res.status(500).json({ error: '获取银行卡片失败，请稍后重试' });
    }
