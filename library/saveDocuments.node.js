...
async function handleSaveDocuments() {
  try {
    ...
    if (!documents || !Array.isArray(documents)) {
      // 返回响应 - res是全局变量
      res.status(400).json({ error: '文档数据格式错误' });
      return;
    }
    
    // 保存文档数据 - 写操作是同步的，不需要await
    db.set(`documents:${session.userId}`, JSON.stringify(documents));
    
    // 返回成功响应 - res是全局变量
    res.status(200).json({ message: '文档数据保存成功', documents: documents });
  } catch (error) {
    console.error('保存文档数据失败:', error);
    // 返回响应 - res是全局变量
    res.status(500).json({ error: '保存文档数据失败，请稍后重试' });
  }
}
...
if (req.method === 'POST') {
  // 处理异步函数的错误
  handleSaveDocuments().catch(error => {
    console.error('保存文档数据处理失败:', error);
    // 如果响应尚未发送，则发送错误响应
    if (!res.headersSent) {
      res.status(500).json({ error: '保存文档数据失败，请稍后重试' });
    }
